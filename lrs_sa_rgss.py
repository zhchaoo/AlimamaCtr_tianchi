...
class LRS_SA_RGSS_combination():
    def __init__(self, clf, df, RecordFolder, LossFunction, columnname, label, ValidSplitParam,
                 start = None, Startcol = ['None'], PotentialAdd = [], CrossMethod = 0):
        self.clf = clf
        ...
        self.ValidSplitParam = ValidSplitParam
        if not self.TemplUsedFeatures:
            self.TemplUsedFeatures = columnname
            if label in self.TemplUsedFeatures:
                self.TemplUsedFeatures.remove(label)
    ...
    def validation(self, selectcol, num, addfeature, coetest=0):#get the score with the new features list and update the best features combination
        ...
        if 1:
            ...
            for D in self.ValidSplitParam[1]:
                T = (X[self.ValidSplitParam[0]] != D)
